...
type 'a asm
  ...
  O parâmetro ['a] é acrescentado como um tipo fantasma. *)
...
val ( ++ ) : ([< `text|`data ] asm as 'a)-> 'a -> 'a
  (** [(++) x y] junta dois pedaços de código [x] [y] ([text] com [text], [data] com [data]). *)
...
val ind: ?ofs:int -> ?index:'size1 register -> ?scale:int ->
  ...
  (** operando indireto [ofs(register, index, scale)]. *)
...
val movabsq: [`Q] operand -> [`Q] register -> text
  (** [movabsq a b] copia um valor direto [a] de 64 bits para um registo [b]. *)
...
(** [add[ b | w | l | q ] a b] adiciona o operando [a] a [b]. Resultado é guardado em [b].*)
...
(** [sub[ b | w | l | q ] a b] subtrai o operando [a] a [b]. Resultado é guardado em [b].*)

val imulw: [`W] operand -> [`W] operand -> text
val imull: [`L] operand -> [`L] operand -> text
val imulq: [`Q] operand -> [`Q] operand -> text
(** [imu[ w | l | q ] a b ] subtrai o operando [a] a [b]. Resultado é guardado em [b].*)

val idivq: [`Q] operand -> text
(** [idiv[ w | l | q ] a b] divisão com sinal de [%rdx::%rax] por [a]. Quociente é guardado em [%rax] e resto em [%rdx].*)

val cqto: text
(* Extende endereço %rax em %rdx::%rax. *)
(** {2 Operações lógicas } *)
...
(** [not[ b | w | l | q ] a] negação binária de [a].*)
...
val shlq: [`Q] operand -> [`Q] operand -> text
  (* [shl[ b | w | l |  q] a b] *shift* lógico para a esquerda. *)
...
val shrq: [`Q] operand -> [`Q] operand -> text
  (* [shr[ b | w | l |  q] a b] *shift* lógico para a direita. *)
...
(* [sar[ b | w | l |  q] a b] *shift* aritmético para a direita. *)
...
val jmp : label -> text
  (** [jmp (z: label)] salto incondicional para a *label* [z].*)
val jmp_star: [`Q] operand -> text
  (** [jmp_star o] salto para um endereço calculado [o].*)
